...
<template>
  <div class="container mx-auto px-4 py-4" v-for="text in savedTexts">
    ...
    <div v-if="!expandedText[text.id]">
      ...
      <!-- read more button -->
      <button @click="readMoreLess(text.id)" class="underline font-medium
          text-sm px-4 py-2 me-2 mb-2 cursor-pointer">Read more</button>
    </div>
    ...
    <div v-if="expandedText[text.id]">
      ...
      <button @click="readMoreLess(text.id)" class="underline font-medium
          text-sm px-5 py-2.5 me-2 mb-2 cursor-pointer">Close</button>
    </div>
