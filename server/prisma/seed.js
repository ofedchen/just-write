...
async function loadLegacyDatabase() {
  ...
  const candidatePaths = [
    ...
    path.resolve(currentDir, "../../src/database.json"),
    "/app/database.json",
    "/src/database.json"
  ].filter(Boolean);
  ...
  throw new Error(
    `Could not find legacy database JSON. Tried: ${candidatePaths.join(", ")}`
  );
}
...
async function main() {
  ...
  console.log(`Hashing ${userInfoByUsername.size} user passwords...`);
  const usersToCreate = [];
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      for (const userData of usersToCreate) {
        ...
        usersByUsername.set(userData.username, createdUser);
        summary.users += 1;
      }
